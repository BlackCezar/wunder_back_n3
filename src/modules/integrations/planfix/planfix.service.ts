...
export class PlanFixService {
    ...
    constructor(
        ...
        private readonly taskService: WrapperType<TasksService>,
    ...
    public async createPreSignUpTask(
        customer: Pick<
            ICustomerCandidate,
            "id" | "publicAgree" | "companyName" | "regionName"
        >
    ): Promise<number> {
        ...
        const description = `Регистрационные данные: ${link}<br /> Клиент по ${customer.publicAgree ? "публичному" : "уникальному"
            } договору`;
    ...
    public async createCompleteSignUpTask(
        ...
        const description = `Проверить регистрационные данные: ${regDataLink}.<br/> Клиент по ${publicAgree ? "публичному" : "уникальному"
            } договору`;
    ...
    public async closeTask(id: string): Promise<boolean> {
        ...
        switch (localTask.type) {
            ...
            case TaskType.NEW_ACCOUNT:
                result = await this.closeAccountTask(
                    localTask,
                    actualPFTask.status
                );
                break;
            ...
            case TaskType.TOP_UP_ACCOUNTS:
                result = await this.closeTopUpTask(
                    localTask,
                    actualPFTask.status
                );
        }
    ...
    private async closePaymentWaitingTask(
        ...
        if (status.id === PlanFixStatus.Completed) {
            ...
            const document =
                await this.invoiceDocumentRepository.getBillDocument(
                    invoice.id
                );
    ...
    private async closeAccountTask(
        ...
        const account = await this.accountRepository.getAccountByTask(
            localTask.id
        );
        const customer = await this.customerRepository.findById<ICustomer>(
            localTask.customerId
    ...
    private async closeCustomerCheckTask(
        ...
        if (status.id === PlanFixStatus.Fail) {
            this.logger.debug(
                "Вызов closeCustomerCheckTask - PlanFixStatus.Fail"
            );
    ...
    private async closeCustomerCandidateTask(
        ...
        const customerCandidate =
            await this.customerCandidateRepository.findById(
        ...
        if (status.id === PlanFixStatus.Fail) {
            ...
            if (customerCandidate.customerId) {
                ...
                const customer =
                    await this.customerRepository.findById<ICustomer>(
                        customerCandidate.customerId
                    );
    ...
    public async createInvoiceWaiting(
        ...
        this.logger.log(
            "Создание задачи PF на ожидание оплаты  " + companyName
        );
