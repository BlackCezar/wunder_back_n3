...
export class AdvertisingService {
    ...
    async getGoogleAdsBalanceRequest(
        ...
        try {
            ...
            const budgetLimit =
                parseFloat(result.accountBudget.adjustedSpendingLimitMicros) /
                1000000;
            const budgetCost =
                parseFloat(result.accountBudget.amountServedMicros ?? "0") /
                1000000;
            return {
                currency: result.customer.currencyCode,
                balance: Math.round(budgetLimit - budgetCost).toLocaleString(
                    "ru"
                ),
            };
        } catch (err) {
    ...
    async getTikTokBadgetBalance(
        ...
        if (response.data.data.advertiser_account_list[0]) {
            return {
                balance:
                    response.data.data.advertiser_account_list[0]
                        .valid_account_balance,
                currency:
                    response.data.data.advertiser_account_list[0].currency,
            };
        }
        return null;
    }
    ...
    async getVKBalance(cliendId: string) {
        ...
        const agencyId = await this.memoryCache.get<string>(
            "vk_agency_account_id"
        );
