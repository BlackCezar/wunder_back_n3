...
import { ContractType, CustomerToSystemSettings, Prisma } from "@prisma/client";
...
export class AccountsTopUpService {
    ...
    private async invoiceDocumentGenerated(
        invoice: IInvoice,
        filename: string
    ) {
        await this.invoiceDocumentRepository.setBillDocument(
            invoice.id,
            filename
        );
