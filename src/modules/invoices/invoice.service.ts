...
import { GenerateInvoiceAct } from "../../common/events/invoice.events";
...
export class InvoiceService {
    ...
    public async createInvoice(payload: CreateInvoiceDto): Promise<IInvoice> {
        ...
        const document =
            await this.invoiceDocumentRepository.createBillDocument(invoice.id);
    ...
    async saveNewDocument(
        payload: Omit<IInvoiceDocument, "id">
    ): Promise<void> {
    ...
    public async updateInvoice(
        ...
        // Update invoice
        let document = await this.invoiceRepository.updateInvoice(
            data.id,
            payload
        );
        ...
        if (data.invoiceDocument?.length) {
            const localDocuments =
                await this.invoiceDocumentRepository.getInvoiceDocuments(
                    data.id
                );
            for (const doc of localDocuments) {
                const oldDoc = data.invoiceDocument.find(
                    (item) => item.id === doc.id
                );
