...
export class RatesService {
  ...
  public async getList(params: RatesFilters): Promise<[number, RateEntity[]]> {
    return Promise.all([this.ratesRepository.getCount(params), this.ratesRepository.getList(params)])
  }
  ...
  private async getRatesByDates(currency: string): Promise<SellRatesEntity | void> {
    let rates: any;
    for (let i = 0; i < 5; i++) {
  ...
  public async fetch(): Promise<void> {
    ...
    await this.grabber.start(region.currency);
  }
