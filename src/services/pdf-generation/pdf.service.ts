...
import {
    ...
    InvoiceEntity,
    IInvoice
} from "../../modules/invoices/entity/invoice.entity";
...
import { SystemName } from "../../modules/systems/entity/system.entity";
import { ExtendedInvoiceLines } from "./pdf.types";
import { RegionNames } from "../../modules/regions/entity/region.entity";
...
export class PdfService {
    ...
    public async generateBill(
        ...
        if (!systemSettings)
            throw new BadRequestException(
                "Настройки систем контракта не найдены"
            );
        ...
        let templateName =
            globalSettings.region.name === RegionNames.KZ
                ? "bill.kz.ejs"
                : "bill.ejs";
        let vatString = this.getVatLabel(globalSettings.region.name, vat);

        let totalString = this.getInvoiceSumLabel(
    ...
    private getInvoiceSum(lines: IInvoiceLine[]): number {
        return lines.reduce((acc, line) => {
            return (acc += line.accounts.reduce((subacc, acc) => subacc += acc.sum, 0));
        }, 0);
    }

    @OnEvent(INVOICE_GENERATE_ACT)
    async generateClosureDocument(invoice: IInvoice, customer: ICustomer): Promise<{ link: string; sum: number; total: number }> {
        ...
        const { settings, contract, systemSettings } = await this.settingsService.getCustomerSettings(customer.id);
        ...
        const templateName = globalSettings.region.name === RegionNames.KZ ? "closure.kz.ejs" : "closure.ejs";
