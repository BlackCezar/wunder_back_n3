...
import { CustomerCandidateRepository } from "../../modules/users/repositories/customer-candidate.repository";
...
export class TasksService {
    ...
    public async createTransferTask(): Promise<void> { }
    ...
    public async signUpTask(
        ...
        const pfTaskId = await this.planFixService.createPreSignUpTask(
            customer
        );
        if (pfTaskId) {
            await this.taskRepository.appendNewTask({
            ...
            await this.customerCandidateRepository.bindTaskId(
                customer.id,
                pfTaskId
            );
        }
