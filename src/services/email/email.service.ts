...
import { IInvoice } from "../../modules/invoices/entity/invoice.entity";
...
export class EmailService {
    ...
    public async createAccountFailedMail(
        ...
        this.logger.log(
            "Письмо провала создания аккаунта отправлено - " +
                customer.contactEmail
        );
    }
    ...
    public async confirmCustomer(
        customer: Pick<
            ICustomer,
            "contactEmail" | "contactName" | "companyEmail"
        >,
        password: string
    ): Promise<void> {
    ...
    public async customerFailComplete(
        ...
        this.logger.log(
            "Отправка письма о проваленной проверке данных: " +
                customer.contactEmail
        );
    ...
    public async topUpAccount(
        ...
        if (result && result.response.search("250") !== -1) {
            this.logger.log(
                "Письмо о пополнении счета отправлено - " +
                    customer.contactEmail
            );
        } else {
            this.logger.debug(
    ...
    public async completeRegistrationMail(
        customer: Pick<
            ICustomerCandidate,
            "id" | "contactEmail" | "contactName"
        >
    ): Promise<void> {
        this.logger.log(
            "Отправка письма о регистрации: " + customer.contactEmail
        );
